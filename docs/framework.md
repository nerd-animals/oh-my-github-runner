...
- Declare the tools the strategy may use in `policies.uses` (e.g. `{ claude: true, codex: true }`). The toolkit refuses any `tk.ai.run({ tool })` not in that set; if `policies.uses` has a single tool, `opts.tool` is optional and inferred.
